...
class FollowtagStaticNode(Node):
    def __init__(self):
        ...
        self.x_offset = [1.7,1.7,1.7,0.8]
        self.z_offset = [-0.7,1.4,0.45,1]
        self.is_seen = False
    ...
    def transform_point(self, point, translation, quaternion):
        if self.follow_id == 1 and self.follow_action == 0:
            x_offset = self.x_offset[0]
            z_offset = self.z_offset[0]
        elif self.follow_id == 5 and self.follow_action == 0:
            x_offset = self.x_offset[1]
            z_offset = self.z_offset[1]
        elif self.follow_id == 6 and self.follow_action == 0:
            x_offset = self.x_offset[2]
            z_offset = self.z_offset[2]
        elif self.follow_id == 6 and self.follow_action == 1:
