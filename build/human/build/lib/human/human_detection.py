...
class ImageAndDepthImageToCoordinateServer(Node):
    def __init__(self):
        super().__init__('human_coordinate_server')
        self.srv = self.create_service(Humancoordinates, 'human_coordinate_server', self.handle_request)
    ...
    def handle_request(self, request, response):
        ...
        depth_frame = request.depth_image
        ...
        if xyxy_list:
            for i in range(len(xyxy_list)):
                if class_id_list[i] == 0:
                    ...
                    coordinate = [x,y,z]
                    coordinate = [round(i,3) for i in coordinate]
                    cv2.circle(canvas,(ux,uy),4,(255,255,255),5) 
                    cv2.putText(canvas,str(coordinate),(ux+10,uy+2),0,0.5,[255,255,255],thickness=1,lineType=cv2.LINE_AA)
                    response.coordinate.x = float(x)
                    response.coordinate.y = float(y)
                    response.coordinate.z = float(z)
...
def main(args=None):
    ...
    server = ImageAndDepthImageToCoordinateServer()
    print("Start server")
    rclpy.spin(server)
    server.destroy_node()
    rclpy.shutdown()
